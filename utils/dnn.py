...
from keras.layers import LSTM as KERAS_LSTM, Dense, Dropout, Conv2D, Flatten, \
    BatchNormalization, Activation, MaxPooling2D, TimeDistributed
...
class DNN(Model):
    ...
    def train(self, x_train, y_train, x_val, y_val, n_epochs, path):
        """
        ...
        self.trained = True
        # best model load
        print('best model load, accuracy is :',best_acc)
        self.load_model(path)
        #
        # checkpoint = ModelCheckpoint('./model-{epoch:03d}-{acc:03f}-{val_acc:03f}.h5',
        #                              verbose=1, monitor='val_loss',
        #                              save_best_only=True, mode='auto')
        # checkpoint = ModelCheckpoint('.mdl_wts.hdf5', save_best_only=True, monitor='val_loss', mode='min')
        # self.model.fit(x_train, y_train, epochs=n_epochs, validation_split=0.3, callbacks=[checkpoint], verbose=False)
        #



    def predict_one(self, sample, type):
        # if not self.trained:
        #     sys.stderr.write(
        #         "Model should be trained or loaded before doing predict\n")
        #     sys.exit(-1)
        # CNN
        if type == 'cnn'or type == 'inter2017':
            sample = np.expand_dims(sample, axis=2)
            if not sample.shape[0] == 198:
                zeropadding  = 198 - sample.shape[0]
                if zeropadding > 0:
                    npad = ((zeropadding, 0), (0, 0), (0, 0))
                    sample = np.pad(sample, npad, 'constant', constant_values=(0))
                else:
                    sample = sample[0:198, :, :]

        # LSTM
        elif type == 'rnn' or type == 'Manon2020':
            if not sample.shape[0] == 198:
                zeropadding = 198 - sample.shape[0]
                if zeropadding > 0:
                    npad = ((zeropadding, 0), (0, 0))
                    sample = np.pad(sample, npad, 'constant', constant_values=(0))
                else:
                    sample = sample[0:198, :]
...
class LSTM(DNN):
    ...
    def make_default_model(self):
        ...
        # print(self.input_shape, self.model.output)
...
class CNN_InterSpeech_2017(DNN):

    def __init__(self, **params):
        params['name'] = 'Inter2017'
        super(CNN_InterSpeech_2017, self).__init__(**params)
    ...
    def make_default_model(self):
        ...
        Makes a CNN keras model with the default hyper parameters.
        """
        self.model.add(Conv2D(16, (12, 16), padding='same',
                       input_shape=(self.input_shape[0], self.input_shape[1], 1)))
        # self.model.add(BatchNormalization(axis=-1))
        # print(self.model.output)
        self.model.add((Activation('relu')))
        self.model.add((MaxPooling2D(pool_size=(2, 1))))
        self.model.add((Conv2D(24, (8, 12), padding='same')))
        # # self.model.add(BatchNormalization(axis=-1))
        # print(self.model.output)
        ...
        self.model.add((Activation('relu')))
        self.model.add((MaxPooling2D(pool_size=(2, 1))))
        self.model.add((Conv2D(32, (5, 7), padding='same')))
        # # self.model.add(BatchNormalization(axis=-1))
        # print(self.model.output)
...
class Manon2020(DNN):
    """
    LSTM based methods (specially Bidirectional LSTM)
    """

    def __init__(self, **params):
        params['name'] = 'Manon2020'
        super(Manon2020, self).__init__(**params)


    def make_default_model(self):
        self.model.add(
            Bidirectional(KERAS_LSTM(200, return_sequences=True),
                          input_shape=(self.input_shape[0], self.input_shape[1])))
        # print(self.input_shape, self.model.output)

        self.model.add(Bidirectional(KERAS_LSTM(64, return_sequences=True, activation='tanh')))
        # print(self.model.output)
        # self.model.add(Dropout(0.5))
        self.model.add(Bidirectional(KERAS_LSTM(32, return_sequences=True, activation='tanh')))
        # print(self.model.output)
        # self.model.add(Dropout(0.5))
        self.model.add(Bidirectional(KERAS_LSTM(32, return_sequences=False, activation='tanh')))
        # print(self.model.output)
        self.model.add(Dropout(0.5))
