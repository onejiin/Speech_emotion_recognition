...
class Model(object):
    ...
    def predict(self, samples: numpy.ndarray, type) -> Tuple:
        """
        ...
        for _, sample in enumerate(samples):
            results.append(self.predict_one(sample, type))
        return tuple(results)

    def predict_one(self, sample, type) -> int:
        """
    ...
    def evaluate(self, x_test: numpy.ndarray, y_test: numpy.ndarray, type) -> None:
        """
        ...
        """
        predictions = self.predict(x_test, type)
        # print(y_test)
        # print(predictions)
